...
export default function MessagesPage() {
  ...
  return (
    <DashboardLayout pageTitle="Messages">
      <div className="bg-white rounded-[28px] border border-slate-100 shadow-sm overflow-hidden flex h-[calc(100vh-180px)] min-h-[500px]">

        {/* ── Contacts Sidebar ──────────────────────────────────────────── */}
        <div className="w-80 border-r border-slate-100 flex flex-col shrink-0 hidden sm:flex">
          {/* Header */}
          <div className="p-5 border-b border-slate-100">
            <h2 className="font-extrabold text-slate-800 text-lg mb-4">Messages</h2>
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-slate-50 rounded-2xl text-sm text-slate-700 placeholder-slate-400 outline-none focus:bg-white focus:ring-2 focus:ring-[#8B5CF6]/20 border border-transparent focus:border-[#8B5CF6]/30 transition-all"
              />
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto">
            {filteredContacts.map((c) => (
              <button
                ...
                className={clsx(
                  "w-full flex items-center gap-3 px-5 py-4 text-left hover:bg-slate-50 transition-colors border-b border-slate-50",
                  activeId === c.id && "bg-purple-50 border-l-4 border-l-[#8B5CF6]"
                )}
              >
                {/* Avatar */}
                <div className="relative shrink-0">
                  <img src={c.avatar} alt={c.name} className="w-11 h-11 rounded-full object-cover bg-slate-100" />
                  {c.online && (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-slate-400 text-[11px] shrink-0 ml-2">{c.time}</span>
                  </div>
                  <div className="flex items-center justify-between mt-0.5">
                    <p className="text-slate-400 text-[12px] truncate">{c.lastMsg}</p>
                    {c.unread > 0 && (
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-white">
            <div className="flex items-center gap-3">
              <div className="relative">
                <img src={activeContact.avatar} alt={activeContact.name} className="w-10 h-10 rounded-full object-cover bg-slate-100" />
            ...
            <div className="flex items-center gap-2">
              <button className="w-9 h-9 rounded-xl hover:bg-slate-100 flex items-center justify-center text-slate-500 hover:text-[#8B5CF6] transition-colors" title="Voice call">
                <Phone size={17} />
              </button>
              <button className="w-9 h-9 rounded-xl hover:bg-slate-100 flex items-center justify-center text-slate-500 hover:text-[#8B5CF6] transition-colors" title="Video call">
                <Video size={17} />
              </button>
              <button className="w-9 h-9 rounded-xl hover:bg-slate-100 flex items-center justify-center text-slate-500 hover:text-[#8B5CF6] transition-colors" title="More options">
            ...
            {activeMessages.map((msg) => (
              ...
              >
                ...
                <div className={clsx(
                  ...
                  msg.from === "me"
                    ? "bg-[#8B5CF6] text-white rounded-br-sm shadow-lg shadow-[#8B5CF6]/20"
                    : "bg-white text-slate-700 rounded-bl-sm shadow-sm border border-slate-100"
                )}>
                  {msg.text}
                  <div className={clsx("flex items-center justify-end gap-1 mt-1", msg.from === "me" ? "text-white/50" : "text-slate-400")}>
          ...
          <div className="px-5 py-4 border-t border-slate-100 bg-white">
            <div className="flex items-center gap-3">
              <button className="text-slate-400 hover:text-[#8B5CF6] transition-colors p-2 shrink-0" title="Attach file">
                <Paperclip size={18} />
              </button>
              <button className="text-slate-400 hover:text-[#8B5CF6] transition-colors p-2 shrink-0" title="Emoji">
                <Smile size={18} />
              </button>
              <input
                id="message-input"
                ...
                className="flex-1 bg-slate-50 rounded-2xl px-4 py-3 text-sm text-slate-700 placeholder-slate-400 outline-none focus:bg-white focus:ring-2 focus:ring-[#8B5CF6]/20 border border-transparent focus:border-[#8B5CF6]/30 transition-all"
              />
              <button
                id="send-message-btn"
                onClick={sendMessage}
